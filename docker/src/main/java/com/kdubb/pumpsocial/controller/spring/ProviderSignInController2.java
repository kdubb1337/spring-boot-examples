 ...
 * <li>GET /signin/{providerId}?oauth_token&oauth_verifier||code - Receive {providerId} authentication callback and
 * establish the
 * connection.</li>
 * </ul>
 * 
...
public class ProviderSignInController2 implements InitializingBean {
	...
	private final static Logger LOG = LogManager.getLogger(ProviderSignInController2.class);
	private final static Log logger = LogFactory.getLog(ProviderSignInController2.class);
	...
	private String signInUrl = "/signin2";
	 ...
	 *            the locator of {@link ConnectionFactory connection factories} used to support provider sign-in. Note:
	 *            this reference
	 *            should be a serializable proxy to a singleton-scoped target instance. This is because
	 *            {@link ProviderSignInAttempt} are
	 *            session-scoped objects that hold ConnectionFactoryLocator references. If these references cannot be
	 *            serialized,
	 *            NotSerializableExceptions can occur at runtime.
	 * @param usersConnectionRepository
	 *            the global store for service provider connections across all users. Note: this reference should be a
	 *            serializable proxy to
	 *            a singleton-scoped target instance.
	 * @param signInAdapter
	 *            handles user sign-in
	 */
	@Inject
	public ProviderSignInController2(ConnectionFactoryLocator connectionFactoryLocator, UsersConnectionRepository usersConnectionRepository,
	...
	/**
	 * Configure the list of sign in interceptors that should receive callbacks during the sign in process. Convenient
	 * when an instance of
	 * this class is configured using a tool that supports JavaBeans-based configuration.
	 * 
	 * @param interceptors
	 ...
	 * <code>https://myapp.com</code>. Defaults to
	 * null. If specified, will be used to generate OAuth callback URLs. If not specified, OAuth callback URLs are
	 * generated from web
	 * request info. You may wish to set this property if requests into your application flow through a proxy to your
	 * application server. In
	 * this case, the request URI may contain a scheme, host, and/or port value that points to an internal server not
	 * appropriate for an
	 * external callback URL. If you have this problem, you can set this property to the base external URL for your
	...
	/**
	 * Sets a strategy to use when persisting information that is to survive past the boundaries of a request. The
	...
	/**
	 * Adds a ConnectInterceptor to receive callbacks during the connection process. Useful for programmatic
	 * configuration.
	 * 
	 * @param interceptor
	...
	/**
	 * Process a sign-in form submission by commencing the process of establishing a connection to the provider on
	 * behalf of the user. For
	 * OAuth1, fetches a new request token from the provider, temporarily stores it in the session, then redirects the
	 * user to the
	 * provider's site for authentication authorization. For OAuth2, redirects the user to the provider's site for
	 * authentication
	 * authorization.
	 */
	@RequestMapping(value = "/google", method = RequestMethod.POST)
	public RedirectView signIn(NativeWebRequest request, Principal currentUser) {
		logger.info("signIn request.getSessionId(): [" + request.getSessionId() + "]");
	...
	public RedirectView signIn(@PathVariable String providerId, NativeWebRequest request) {
		logger.info("Signing in with FB");
	...
	/**
	 * Process the authentication callback from an OAuth 1 service provider. Called after the member authorizes the
	 * authentication,
	 * generally done once by having he or she click "Allow" in their web browser at the provider's site. Handles the
	 * provider sign-in
	 * callback by first determining if a local user account is associated with the connected provider account. If so,
	 * signs the local user
	 * in by delegating to {@link SignInAdapter#signIn(String, Connection, NativeWebRequest)} If not, redirects the user
	 * to a signup page to
	 * create a new account with {@link ProviderSignInAttempt} context exposed in the HttpSession.
	 * 
	...
	/**
	 * Process the authentication callback from an OAuth 2 service provider. Called after the user authorizes the
	 * authentication, generally
	 * done once by having he or she click "Allow" in their web browser at the provider's site. Handles the provider
	 * sign-in callback by
	 * first determining if a local user account is associated with the connected provider account. If so, signs the
	 * local user in by
	 * delegating to {@link SignInAdapter#signIn(String, Connection, NativeWebRequest)}. If not, redirects the user to a
	 * signup page to
	 * create a new account with {@link ProviderSignInAttempt} context exposed in the HttpSession.
	 * 
	 * @see ProviderSignInAttempt
	 ...
	 * http://tools.ietf.org/html/rfc6749#section-4.1.2.1. Called
	 * after upon redirect from an OAuth 2 provider when there is some sort of error during authorization, typically
	 * because the user denied
	 * authorization. Simply carries the error parameters through to the sign-in page.
	 */
	...
	/**
	 * Process the authentication callback when neither the oauth_token or code parameter is given, likely indicating
	 * that the user denied
	 * authorization with the provider. Redirects to application's sign in URL, as set in the signInUrl property.
	 */
	...
	private RedirectView handleSignIn(Connection<?> connection, ConnectionFactory<?> connectionFactory, NativeWebRequest request) {
		...
		logger.info("RedirectView handleSignIn");
		if (userIds.size() == 0) {
			...
			MongoConnection mongoConnection = connectionConverter.convert(connection);
			...
			// TODO make this async and save last login time
			// Refresh connections
			// refreshConnectionService.refreshAll(userId);
