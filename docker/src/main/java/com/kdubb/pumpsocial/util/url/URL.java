package com.kdubb.pumpsocial.util.url;
...
public class URL implements Iterable<URL.Parameter> {
	...
	public class Parameter {
		...
		/**
		 * Creates a parameter with the provided name and an empty value. The
		 * name is encoded upon storage.
		 * 
		 * @param name
		...
		/**
		 * Creates a parameter with the provided name and value, both of which
		 * are encoded upon storage.
		 * 
		 * @param name
		 *            The name of the parameter. Silently accepts null and empty
		 *            strings, however a blank parameter will be returned when
		 *            toString() is called.
		 * @param value
		 *            The value of the parameter. Accepts null and empty
		 *            strings.
		 */
		 ...
		 * name=value If the name is empty an empty string is returned. If the
		 * value is empty, the name alone is returned.
		 */
		@Override
	 ...
	 *            The external form of a URL.
	 * @param defaul
	 *            The URL that should be returned in cases where the primary url
	 *            is invalid.
	 * @return A URL object for the provided String, or null if the URL provided
	 *         is invalid.
	 */
	 ...
	 * is that the calling method will check the url for correctness. To help
	 * with this, you can use the static method:
	 * 
	 * <pre>
	 ...
	 * 
	 * @param url
	 *            The external form of the URL. Ex: http://www.google.com If
	 *            this parameter is empty or invalid an IllegalArgumentException
	 *            is thrown. If this parameter is null a NullPointerException.
		...
		try {
			...
			if (u.getHost() == null) {
				throw new IllegalArgumentException(
						"The class can only represent URLs.");
			}
	 ...
	 * </pre>
	 * 
	 * <b>NOTE:</b> Adding a parameter that already exists is supported.
	 * Example:
	 * 
	 * <pre>
	...
	/**
	 * Decrements by 1 the integer value of the parameter in the URL that
	 * matches the provided name. If the URL contains multiple parameters, the
	 * first encountered that can be casted to an int will be decremented. If no
	 * parameter exists that equals the provided name AND can be casted to an
	 * int, the url will not be modified.
	 * 
	 * @param name
	...
	public boolean equals(Object anObject) {
		...
		if (anObject instanceof java.net.URL) {
			return (toString().equals(((java.net.URL) anObject)
					.toExternalForm()));
		}
	...
	private String fragmentDecode(String value) {
		try {
			...
			throw new IllegalArgumentException(URL.ENCODING
					+ URL.ENCODING_ERROR, e);
		}
	}
	 ...
	 * encountered that can be casted to an int will be returned. If no
	 * parameter exists with the provided name AND cannot be casted to an int,
	 * the provided defaul int value will be returned.
	 * 
	 * @param name
	 *            The name of the parameter to return. The name is automatically
	 *            encoded if necessary.
	 * @param defaul
	 *            If the is no parameter to return, this value is returned
	 *            instead.
	...
	/**
	 * Gets the value of the parameter in the URL that matches the provided
	 * name. If the URL contains multiple parameters, the first encountered will
	 * be returned. In cases where the URL contains multiple parameters with the
	 * same name, used getParameters(String name) instead. If no parameter
	 * exists with the provided name, the provided defaul will be returned. If
	 * the parameter has no value, "www.example.com/?test", an empty string is
	 * returned, <b>not the defaul value</b>.
	 * 
	 * @param name
	 *            The name of the parameter to return. The name is automatically
	 *            encoded if necessary.
	...
	/**
	 * Gets the values of the parameters in the URL that matche the provided
	 * name. If the URL contains unique parameter names, use getPatameter
	 * instead. If no parameter exists with the provided name, an empty array
	 * will be returned.
	 * 
	 * @param name
	 *            The name of the parameter(s) to return. The name is
	...
	public List<String> getPathElements() {
		...
		/**
		 * Sometimes paths are formed poorly with double slashes This regex
		 * makes sure doulbe slashes are treated as a single slash.
		 */
	...
	/**
	 * Returns the parent directory of a directory in this URLs path. For
	 * example, if this url's path is: /a/b/c/d.html all of these statements are
	 * true:
	 * 
	 * <pre>
	 ...
	 * </pre>
	 * 
	 * In all fault cases and empty string is returned, including: this url's
	 * path is empty/root ("/"), if the provided directory or file is not found,
	 * the parent category is root.
	 */
	...
	/**
	 * Returns the child directory (or filename) of a directory in this URLs
	 * path. For example, if this url's path is: /a/b/c/d.html all of these
	 * statements are true:
	 * 
	 * <pre>
	 ...
	 * In all fault cases and empty string is returned, including: this url's
	 * path is empty/root ("/"), if the provided directory or file is not found,
	 * the matching element is the last element in the path.
	 */
	...
	/**
	 * Returns the port of this URL. If no port is present in the URL, the
	 * default for the scheme is provided. For example, 80 from:
	 * http://www.google.com/?name=value#fragment
	 * 
	 ...
	 * protocol://username:password@host:port/path?query#fragment
	 * 
	 * @return The port of this url. If no other port is provided, the
	 *         protocol's default port will be returned. (Example: 80 for http)
	 */
	public int getPort() {
		return port;
	}
	...
	/**
	 * Returns the protocol portion of this URL. For example, the enum for http
	 * is returned from: http://www.google.com/
	 * 
	 * For reference the get methods are named as:
	 * protocol://username:password@host:port/path?query#fragment
	 * 
	 ...
	 * calling this method with "/a.html" will return
	...
	/**
	 * Increments by 1 the integer value of the parameter in the URL that
	 * matches the provided name. If the URL contains multiple parameters, the
	 * first encountered that can be casted to an int will be incremented. If no
	 * parameter exists with the provided name AND cannot be casted to an int,
	 * the parameter will be added with a value of 2.
	 * 
	 ...
	 * http://www.google.com/?param1=value1&param2=value2 - this iterator would
	 * iterate over param2 and param2.
	 * 
	...
	/**
	 * Sets the fragment for this URL. Any existing fragment will be
	 * overwritten. A null or empty parameter will remove the fragment
	 * completely.
	 * 
	 * @param fragment
	 * @return This object for chaining.
	 ...
	 * </pre>
	 * 
	 * If more than one parameter already exists with this name, the resulting
	 * url will only contain one parameter with this name and value.
	 * 
	 * Example:
	 * 
	 * <pre>
	 ...
	 * 
	 * @param protocol
	 *            The protocol name, in any case, without :// Examples: http,
	 *            HTTPS, ftp
	 * @return This object for chaining.
	 * @throws IllegalArgumentException
	 *             If not such protocol is supported, IllegalArgumentException
	 *             is thrown.
	 */
	 ...
	 * the domain. For example, http://www.shopobot.com/search?q=1 is returned
	 * as /search?q=1
	 */
	 ...
	 * leading "?". If there are no query parameters, an empty string is
	 * returned.
	 */
	...
	/**
	 * Helper method that just returns the fragment string of this URL,
	 * including a leading "#". If there is no fragment, an empty string is
	 * returned.
	 */
	...
	public String toStringRelative(URL relativeTo) {
		...
		// first we need to verify that all items preceding the path are
		// identical
		...
		String sourceElements[] = source.isEmpty() ? new String[0] : source
				.split("/");
		String targetElements[] = target.isEmpty() ? new String[0] : target
				.split("/");

		// set int common to the number of common elements in the path
		int common = 0;
		for (common = 0; common < sourceElements.length
				&& common < targetElements.length; common++) {
		...
		for (int i = common; i < targetElements.length; i++) {
			relative += (relative.endsWith("../") || relative.isEmpty()) ? targetElements[i]
					: "/" + targetElements[i];
		}

		// since we remove the trailing slash, add it back in if requested,
		// unless
		...
		if (getQueryString().isEmpty()
				&& !relativeTo.getQueryString().isEmpty()) {
		...
		} else if (!this.getFragmentString().equals(
				relativeTo.getFragmentString())) {
	 ...
	 * 
	 * @param queryString
	 *            Either a whole URL ("http://www.shopobot.com/?query=test") or
	 *            just a query string ("?query=test"). If null or "" is pased,
	 *            this method silently returns without changing anything.
	 */
	private URL parseQueryString(String queryString) {
		...
		if (pos > -1) {
			...
			for (String param : query.split("&")) {
				...
				if (pos > -1) {
					// regular parameter
					addParameterNoEncode(param.substring(0, pos),
	...
	/**
	 * Returns an instance of java.net.URL that represents this URL. If the URL
	 * is malformed, null is returned.
	 */
	 ...
	 * new URL("www.subdomain.example.com").matchesAuthority(
	 * "subdomain.example.com"); //true new
	 * URL("www.subdomain.example.com").matchesAuthority(
	 * "www.subdomain.example.com"); //true
	 * 
	 * @param string
	 *            A string reresenting the authority to check against this url.
	 * @return true if the authority matches, false in all other cases
	 *         (including if a null or empty parameter is passed).
	 */
	...
	/**
	 * Checks if any of the authorities provided match this URL. If one matches,
	 * true is returned.
	 */
	public boolean matchesAuthority(String... authority) {
